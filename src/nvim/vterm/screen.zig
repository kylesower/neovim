...
pub export fn vterm_screen_free(screen: *VTermScreen) callconv(.c) void {
  if (screen.buffers[BUFIDX_PRIMARY]) |buf| {
    vterm_allocator_free(screen.vt, buf);
  }
  if (screen.buffers[BUFIDX_ALTSCREEN]) |buf| {
    vterm_allocator_free(screen.vt, buf);
  }
